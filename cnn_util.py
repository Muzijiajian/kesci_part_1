...
deploy = '/home/lord/caffe-master/models/bvlc_reference_caffenet/deploy.prototxt'
model = '/home/lord/caffe-master/models/bvlc_reference_caffenet/bvlc_reference_caffenet.caffemodel'
mean = '/home/lord/caffe-master/python/caffe/imagenet/ilsvrc_2012_mean.npy'
...
class CNN(object):
    ...
    def get_features(self, image_list, layers='fc7', layer_sizes=[4096]):
        ...
        for start, end in zip(range(0, iter_until, self.batch_size), \
            ...
            for image in image_batch_file:
                tmp_image = crop_image(image, target_width=self.width, target_height=self.height)
            ...
            print 'wahaha', image_batch.shape
    ...
    def get_probs(self, image_list, layers='fc7', layer_sizes=[4096]):
        iter_until = len(image_list) + self.batch_size
        all_feats = np.zeros([len(image_list)] + layer_sizes)
        ...
        for start, end in zip(range(0, iter_until, self.batch_size), \
            ...
            image_batch_file = image_list[start:end]
            print 'haha', len(image_batch_file)
